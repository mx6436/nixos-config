{
  description = "A reproducible script to input text into games via clipboard injection";
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      packages.${system}.default = pkgs.writeShellApplication {
        ...
        text = ''
          # 1. 休眠 0.4 秒 (等待 zenity 窗口弹出并获得焦点)
          # 2. 调用 fcitx5-remote -o (打开输入法/切换到中文)
          (sleep 0.4 && fcitx5-remote -s wbx) &
      ...
      apps.${system}.default = {
        type = "app";
        program = "${self.packages.${system}.default}/bin/game-input-helper";
      };
    };
}
